...
public class AlertResource {
  ...
  return Response.ok(jsonString, MediaType.APPLICATION_JSON).build();
	...
	try{
		String query_getAlerts = "SELECT `Alerts` FROM Users where `UserID` = ?";
		prep_sql = link.linea.prepareStatement(query_getAlerts);
		...
		ResultSet rs_query_getAlerts= prep_sql.executeQuery();
			...
			while(rs_query_getAlerts.next()){
				...
				alertEnable=rs_query_getAlerts.getBoolean("Alerts");
	...
	if(alertEnable){
		...
		SendEmail sendEmail = new SendEmail(currentUser.getEmail(), "DynoCloud Alerts", alert.getMessage());
		sendEmail.send();
	...
	public Response getAlertSettings(@Context HttpHeaders headers) {
	  ...
	  AlertSettings alert = new AlertSettings();
		
		try{
			String query_getSettings = "SELECT `Alerts` FROM `Users` where `UserID` = ?";
			...
			ResultSet rs_query_getSettings= prep_sql.executeQuery();
				...
				while(rs_query_getSettings.next()){
					...
					alert.setAlert(rs_query_getSettings.getBoolean("Alerts"));
...
//	}
	...
	@Path("settings")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response updateUser(AlertSettings alert, @Context HttpHeaders headers) {
      ...
      System.out.println("["+currentUser.getUserName()+"] [PUT] /alerts/settings");
		...
		try{
			String query_updateSettings = "UPDATE Users SET `Alerts`=? WHERE `UserID`=?;";
			prep_sql = link.linea.prepareStatement(query_updateSettings);
							
			prep_sql.setBoolean(1, alert.getAlert());
